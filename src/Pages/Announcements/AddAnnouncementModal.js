...
function AddAnnouncementModal({ isOpen, onClose }) {
  const authToken = Cookies.get("authToken");
  const { addStatus } = useSelector((state) => state.announcements);
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} size="xl" scrollBehavior="inside">
      <ModalOverlay />
      <ModalContent>
        <ModalHeader className="text-xl font-semibold">Send Announcement</ModalHeader>
        <ModalCloseButton />
        <form onSubmit={formik.handleSubmit}>
          ...
          <ModalFooter gap={2}>
            ...
            <Button
              ...
              loadingText="Sending..."
              isDisabled={!canSubmit && addStatus !== "loading"}
            >
              Send Announcement
            </Button>
          </ModalFooter>
